...
func TestDeliveryAuthenticatesScopesAndAvoidsSecretLogs(t *testing.T) {
	...
	app, _ := New(store, mailer, []config.Client{{ID: "c6s", Token: "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG", Templates: []string{mailtemplate.CerberusInvitation}}}, slog.New(slog.NewJSONHandler(&logs, nil)))
	body := `{"template":"cerberus.organization-invitation","recipient":"person@example.com","locale":"en","variables":{"invitationLink":"https://console.c6s.whitekiwi.link/invitations/accept/#token=secret-token-value"}}`
	...
	if response.Code != http.StatusAccepted || store.completed == "" || mailer.message.Recipient != "person@example.com" {
